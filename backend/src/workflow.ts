...
// console.log("console of getContext 👍",jokeFlow);
...
async function main() {
  ...
  sendEvent(startEvent.with("corporate"));
...
app.post("/joke",async(req,res)=>{
    try{
        console.log("I am inside workflow setup file");
        return res.status(200).json({message:"Joke workflow started successfully!"});
    }
    catch(error){
